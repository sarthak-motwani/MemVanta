...
void rope(std::vector<float>& q, std::vector<float>& k, std::size_t pos, std::size_t heads) {
    ...
    for (std::size_t head=0; head<heads; ++head) {
        ...
        for(std::size_t i=0;i+1<hd;i+=2) {
            ...
            auto rot=[&](std::vector<float>& z){ float x0=z[b+i], x1=z[b+i+1]; z[b+i]=x0*c-x1*s; z[b+i+1]=x0*s+x1*c; };
            rot(q); rot(k);
        }
    }
}
...
public:
    explicit MiniLlama(const Config& c): c_(c), lm_(c.vocab,c.hidden,0xABCDEF) {
        if (c.hidden%32 || c.ff%32 || c.hidden%c.heads) throw std::runtime_error("invalid dimensions");
        layers_.reserve(c.layers);
        for(std::size_t i=0;i<c.layers;++i) layers_.emplace_back(c.hidden,c.ff,0x10000+i*17);
        kcache_.resize(c.layers*c.max_ctx*c.hidden);
        vcache_.resize(c.layers*c.max_ctx*c.hidden);
        x_.resize(c.hidden); n_.resize(c.hidden); q_.resize(c.hidden); k_.resize(c.hidden); v_.resize(c.hidden);
    ...
    int step(int token, bool logits=true) {
        ...
        for(std::size_t li=0;li<c_.layers;++li){
            ...
            std::memcpy(kc,k_.data(),c_.hidden*sizeof(float)); std::memcpy(vc,v_.data(),c_.hidden*sizeof(float));
            ...
            for(std::size_t head=0;head<c_.heads;++head){
                ...
                for(std::size_t t=0;t<=pos_;++t){
                    const float* kt=&kcache_[(li*c_.max_ctx+t)*c_.hidden+hb]; float s=0;
                ...
                for(std::size_t t=0;t<=pos_;++t){
                    const float a=score[t]*inv; const float* vt=&vcache_[(li*c_.max_ctx+t)*c_.hidden+hb];
...
int main_impl(int argc,char**argv){
    ...
    for(unsigned i=0;i<c.warmup;++i){ (void)pp(32); (void)tg(8); }
    Result r;
    for(unsigned rep=0;rep<c.reps;++rep){r.pp_tps.push_back(pp(512));r.tg_tps.push_back(tg(128));auto [tt,tps]=client();r.ttft_ms.push_back(tt);r.out_tps.push_back(tps);}
    const auto spp=calc(r.pp_tps), stg=calc(r.tg_tps), stt=calc(r.ttft_ms), sot=calc(r.out_tps);
    ...
    std::cout<<"| MemVanta Synthetic Llama Q8_0 | "<<std::fixed<<std::setprecision(2)<<mib<<" MiB | "<<m.params()/1e6<<" M | CPU/AVX2 | "<<c.threads<<" | pp512 | "<<spp.mean<<" ± "<<spp.sd<<" t/s |\n";
    std::cout<<"| MemVanta Synthetic Llama Q8_0 | "<<mib<<" MiB | "<<m.params()/1e6<<" M | CPU/AVX2 | "<<c.threads<<" | tg128 | "<<stg.mean<<" ± "<<stg.sd<<" t/s |\n";
    ...
    if(!csv.empty()){
        ...
        for(std::size_t i=0;i<r.pp_tps.size();++i)f<<i+1<<","<<r.pp_tps[i]<<","<<r.tg_tps[i]<<","<<r.ttft_ms[i]<<","<<r.out_tps[i]<<"\n";
    }
    return 0;
}
}
