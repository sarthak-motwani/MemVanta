...
BATCH=${BATCH:-8}
KV=${KV:-f16}
MEMVANTA_TIMEOUT=${MEMVANTA_TIMEOUT:-600}
...
echo "[diagnostic] starting MemVanta benchmark at $(date -u +'%Y-%m-%dT%H:%M:%SZ')" | tee "$OUT/progress.txt"
set +e
...
mem_rc=$?
set -e
echo "[diagnostic] MemVanta benchmark rc=$mem_rc finished at $(date -u +'%Y-%m-%dT%H:%M:%SZ')" | tee -a "$OUT/progress.txt"
...
echo "[diagnostic] MemVanta greedy rc=$greedy_rc" | tee -a "$OUT/progress.txt"

if [[ -x "$LLAMA_BIN/llama-bench" ]]; then
  echo "[diagnostic] starting llama.cpp benchmark at $(date -u +'%Y-%m-%dT%H:%M:%SZ')" | tee -a "$OUT/progress.txt"
  set +e
  ...
  echo "[diagnostic] llama.cpp benchmark rc=$llama_rc finished at $(date -u +'%Y-%m-%dT%H:%M:%SZ')" | tee -a "$OUT/progress.txt"
else
...
if [[ -x "$LLAMA_BIN/llama-cli" ]]; then
  ...
  echo "$?" > "$OUT/llama.greedy.exitcode.txt"
  set -e
fi

python3 scripts/summarize_ab.py "$OUT" || true
echo "results: $OUT"
exit "$status"
