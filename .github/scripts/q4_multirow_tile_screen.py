...
ROOT=Path(__file__).resolve().parents[2]; SRC=ROOT/'src/gguf_kernels.cpp'
MODEL=ROOT/os.environ.get('MODEL','stories15M-q4_0.gguf'); RESULT=ROOT/'results/q4-multirow-tile-screen'; REPS=5

def run(cmd,**kw): subprocess.run(cmd,cwd=ROOT,check=True,**kw)
def build(name):
    b=ROOT/f'build-{name}'; run(['cmake','-S','.','-B',str(b),'-G','Ninja','-DCMAKE_BUILD_TYPE=Release']); run(['cmake','--build',str(b),'--target','memvanta_profile','memvanta_tests','memvanta_tokenizer_tests','-j',str(os.cpu_count() or 2)]); run(['ctest','--test-dir',str(b),'--output-on-failure']); return b
...
def patch(text):
    ...
    for(std::size_t bi=0;bi<nb;++bi){
        ...
        const __m128i p0=_mm_loadu_si128(reinterpret_cast<const __m128i*>(w0[bi].qs)),p1=_mm_loadu_si128(reinterpret_cast<const __m128i*>(w1[bi].qs));
        ...
        for(int h=0;h<2;++h){const __m128i s0=h?h0:l0,s1=h?h1:l1;for(int k=0;k<16;k+=8){
            const std::size_t off=bi*32+h*16+k; const __m256 x0=_mm256_loadu_ps(x+off),x1=_mm256_loadu_ps(x+stride+off),x2=_mm256_loadu_ps(x+2*stride+off),x3=_mm256_loadu_ps(x+3*stride+off);
            const __m256 q0=_mm256_mul_ps(_mm256_cvtepi32_ps(_mm256_cvtepi8_epi32(_mm_srli_si128(s0,k))),_mm256_set1_ps(d0));
            const __m256 q1=_mm256_mul_ps(_mm256_cvtepi32_ps(_mm256_cvtepi8_epi32(_mm_srli_si128(s1,k))),_mm256_set1_ps(d1));
            a00=_mm256_fmadd_ps(q0,x0,a00);a01=_mm256_fmadd_ps(q0,x1,a01);a02=_mm256_fmadd_ps(q0,x2,a02);a03=_mm256_fmadd_ps(q0,x3,a03);
            a10=_mm256_fmadd_ps(q1,x0,a10);a11=_mm256_fmadd_ps(q1,x1,a11);a12=_mm256_fmadd_ps(q1,x2,a12);a13=_mm256_fmadd_ps(q1,x3,a13);
        }}}
    }
...
def bench(build0,build1):
    RESULT.mkdir(parents=True,exist_ok=True); variants=['baseline','rows2_ffn','rows2_all']; data={v:[] for v in variants}; order=[]; rng=random.Random(20260822)
    for _ in range(REPS): b=variants[:];rng.shuffle(b);order+=b
    ...
    for v in order:
        ...
        if v!='baseline': exe=build1/'memvanta_profile';env['MEMVANTA_MULTIROW_MODE']='ffn' if v=='rows2_ffn' else 'all'
        ...
        m=dict((k,float(x)) for k,x in mr.findall(p.stdout));q=rr.search(p.stderr);data[v].append((m['prefill_ms'],m['decode_total_ms'],float(q.group(1))))
    def agg(v):
        x=data[v];pm=[z[0] for z in x];dm=[z[1] for z in x];rm=[z[2] for z in x];return {'prefill_ms':statistics.mean(pm),'decode_ms':statistics.mean(dm),'rss_kib':statistics.mean(rm),'prefill_cv':100*statistics.stdev(pm)/statistics.mean(pm),'decode_cv':100*statistics.stdev(dm)/statistics.mean(dm)}
    ...
    for v,r in rows.items():
        r['prefill_tps']=64000/r['prefill_ms'];r['decode_tps']=16000/r['decode_ms'];r['gain']=(score(r)/bs-1)*100;r['rss_delta']=(r['rss_kib']/base['rss_kib']-1)*100;r['decode_delta']=(r['decode_tps']/bd-1)*100;r['pass']=v!='baseline' and r['gain']>=5 and r['rss_delta']<=2 and base['prefill_cv']<=5 and base['decode_cv']<=5 and r['prefill_cv']<=5 and r['decode_cv']<=5 and r['decode_delta']>=-2
        if r['pass']:passed.append((score(r),v))
    winner=max(passed)[1] if passed else None;ranked=sorted(variants,key=lambda v:score(rows[v]),reverse=True);(RESULT/'summary.json').write_text(json.dumps({'winner':winner,'rows':rows},indent=2)+'\n')
    ...
    for i,v in enumerate(ranked,1): r=rows[v];lines.append(f"| {i} | {v} | {r['prefill_tps']:.2f} | {r['decode_tps']:.2f} | {r['gain']:.2f}% | {r['rss_delta']:.2f}% | {r['prefill_cv']:.2f}% | {r['decode_cv']:.2f}% | {'YES' if r['pass'] else 'no'} |")
    lines+=['',f'Winner: **{winner or "none"}**.','Gate: >=5% combined gain, <=2% RSS increase, <=5% timing CV baseline/candidate, <=2% decode regression.'];(RESULT/'SUMMARY.md').write_text('\n'.join(lines)+'\n');print('\n'.join(lines))

def main():
    original=SRC.read_text(); b0=build('mr-baseline')
    try: SRC.write_text(patch(original));b1=build('mr-candidate')
