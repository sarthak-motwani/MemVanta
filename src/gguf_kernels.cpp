...
void parallel_rows(std::size_t rows, unsigned threads, WorkerPool* pool, Fn fn) {
    ...
    // Decode historically passed pool=nullptr, which opened an OpenMP region (or
    // spawned std::threads) for every Q/K/V/O/FFN/output matvec. Keep a small
    // thread-local persistent pool so repeated single-token GEMV calls reuse the
    // same workers. Explicit model pools still take precedence for batched work.
    if(!pool && threads>1){
        thread_local unsigned cached_threads=0;
        thread_local std::unique_ptr<WorkerPool> cached_pool;
        if(!cached_pool || cached_threads!=threads){
            cached_pool=std::make_unique<WorkerPool>(threads);
            cached_threads=threads;
        }
        pool=cached_pool.get();
    }
...
inline int hsum_i32(__m256i v){
    __m128i lo=_mm256_castsi256_si128(v),hi=_mm256_extracti128_si256(v,1);
    __m128i s=_mm_add_epi32(lo,hi);s=_mm_hadd_epi32(s,s);s=_mm_hadd_epi32(s,s);return _mm_cvtsi128_si32(s);
}
#endif
...
#if defined(__AVX2__)
        __m256 acc=_mm256_setzero_ps();for(int k=0;k<32;k+=8){auto q8=_mm_loadl_epi64(reinterpret_cast<const __m128i*>(blocks[b].qs+k));auto qi=_mm256_cvtepi8_epi32(q8);acc=_mm256_fmadd_ps(_mm256_cvtepi32_ps(qi),_mm256_loadu_ps(x+b*32+k),acc);}sum+=d*hsum_ps(acc);
#else
...
void quantize_q8_into(const float* x,std::size_t cols,Q8Vector& a){
    if(cols%32)throw std::runtime_error("Q8 activation quantization requires cols multiple of 32");
    a.q.resize(cols);a.d.resize(cols/32);
...
Q8ActBatch quantize_activations_q8(const float*x,std::size_t batch,std::size_t cols){
    Q8ActBatch a;a.batch=batch;a.cols=cols;a.blocks=cols/32;a.q.resize(batch*cols);a.d.resize(batch*a.blocks);Q8Vector tmp;tmp.q.reserve(cols);tmp.d.reserve(a.blocks);
    for(std::size_t b=0;b<batch;++b){quantize_q8_into(x+b*cols,cols,tmp);std::memcpy(a.q.data()+b*cols,tmp.q.data(),cols);std::memcpy(a.d.data()+b*a.blocks,tmp.d.data(),a.blocks*sizeof(float));}return a;
}
...
inline int dot_q4_q8_block(const GgufBlockQ4_0&w,const std::int8_t*aq){
    ...
    const __m128i a0=_mm_loadu_si128(reinterpret_cast<const __m128i*>(aq));
    const __m128i a1=_mm_loadu_si128(reinterpret_cast<const __m128i*>(aq+16));
    const __m256i p0=_mm256_madd_epi16(_mm256_cvtepi8_epi16(lo8),_mm256_cvtepi8_epi16(a0));
    const __m256i p1=_mm256_madd_epi16(_mm256_cvtepi8_epi16(hi8),_mm256_cvtepi8_epi16(a1));
    return hsum_i32(_mm256_add_epi32(p0,p1));
#else
    int s=0;for(int i=0;i<16;++i){s+=(int(w.qs[i]&15)-8)*int(aq[i]);s+=(int(w.qs[i]>>4)-8)*int(aq[i+16]);}return s;
#endif
}
...
inline void q4_row_batch4_fp32(const GgufBlockQ4_0*w,const float*x,std::size_t stride,std::size_t nb,float out[4]){
    ...
    out[0]=hsum_ps(a0);out[1]=hsum_ps(a1);out[2]=hsum_ps(a2);out[3]=hsum_ps(a3);
#else
...
#if defined(__AVX2__)
    __m256 a0=_mm256_setzero_ps(),a1=_mm256_setzero_ps(),a2=_mm256_setzero_ps(),a3=_mm256_setzero_ps(),a4=_mm256_setzero_ps(),a5=_mm256_setzero_ps(),a6=_mm256_setzero_ps(),a7=_mm256_setzero_ps();const __m128i mask=_mm_set1_epi8(0x0f),bias=_mm_set1_epi8(8);
    for(std::size_t bi=0;bi<nb;++bi){const __m256 ds=_mm256_set1_ps(fp16_to_fp32(w[bi].d));const __m128i packed=_mm_loadu_si128(reinterpret_cast<const __m128i*>(w[bi].qs));const __m128i lo=_mm_sub_epi8(_mm_and_si128(packed,mask),bias),hi=_mm_sub_epi8(_mm_and_si128(_mm_srli_epi16(packed,4),mask),bias);for(int h=0;h<2;++h){const __m128i src=h?hi:lo;for(int k=0;k<16;k+=8){__m256 q=_mm256_mul_ps(_mm256_cvtepi32_ps(_mm256_cvtepi8_epi32(_mm_srli_si128(src,k))),ds);std::size_t off=bi*32+h*16+k;a0=_mm256_fmadd_ps(q,_mm256_loadu_ps(x+off),a0);a1=_mm256_fmadd_ps(q,_mm256_loadu_ps(x+stride+off),a1);a2=_mm256_fmadd_ps(q,_mm256_loadu_ps(x+2*stride+off),a2);a3=_mm256_fmadd_ps(q,_mm256_loadu_ps(x+3*stride+off),a3);a4=_mm256_fmadd_ps(q,_mm256_loadu_ps(x+4*stride+off),a4);a5=_mm256_fmadd_ps(q,_mm256_loadu_ps(x+5*stride+off),a5);a6=_mm256_fmadd_ps(q,_mm256_loadu_ps(x+6*stride+off),a6);a7=_mm256_fmadd_ps(q,_mm256_loadu_ps(x+7*stride+off),a7);}}}
    out[0]=hsum_ps(a0);out[1]=hsum_ps(a1);out[2]=hsum_ps(a2);out[3]=hsum_ps(a3);out[4]=hsum_ps(a4);out[5]=hsum_ps(a5);out[6]=hsum_ps(a6);out[7]=hsum_ps(a7);
#else
...
#if defined(__AVX2__)
    __m256 a0=_mm256_setzero_ps(),a1=_mm256_setzero_ps(),a2=_mm256_setzero_ps(),a3=_mm256_setzero_ps(),a4=_mm256_setzero_ps(),a5=_mm256_setzero_ps(),a6=_mm256_setzero_ps(),a7=_mm256_setzero_ps();
    for(std::size_t bi=0;bi<nb;++bi){const __m256 ds=_mm256_set1_ps(fp16_to_fp32(w[bi].d));for(int k=0;k<32;k+=8){__m128i q8=_mm_loadl_epi64(reinterpret_cast<const __m128i*>(w[bi].qs+k));__m256 q=_mm256_mul_ps(_mm256_cvtepi32_ps(_mm256_cvtepi8_epi32(q8)),ds);std::size_t off=bi*32+k;a0=_mm256_fmadd_ps(q,_mm256_loadu_ps(x+off),a0);a1=_mm256_fmadd_ps(q,_mm256_loadu_ps(x+stride+off),a1);a2=_mm256_fmadd_ps(q,_mm256_loadu_ps(x+2*stride+off),a2);a3=_mm256_fmadd_ps(q,_mm256_loadu_ps(x+3*stride+off),a3);a4=_mm256_fmadd_ps(q,_mm256_loadu_ps(x+4*stride+off),a4);a5=_mm256_fmadd_ps(q,_mm256_loadu_ps(x+5*stride+off),a5);a6=_mm256_fmadd_ps(q,_mm256_loadu_ps(x+6*stride+off),a6);a7=_mm256_fmadd_ps(q,_mm256_loadu_ps(x+7*stride+off),a7);}}
    out[0]=hsum_ps(a0);out[1]=hsum_ps(a1);out[2]=hsum_ps(a2);out[3]=hsum_ps(a3);out[4]=hsum_ps(a4);out[5]=hsum_ps(a5);out[6]=hsum_ps(a6);out[7]=hsum_ps(a7);
#else
    for(int j=0;j<8;++j)out[j]=dot_q8_0_fp32(w,x+j*stride,nb*32);
#endif
}
} // namespace
...
#if defined(__AVX2__)
    __m256 acc=_mm256_setzero_ps();std::size_t i=0;for(;i+8<=n;i+=8)acc=_mm256_fmadd_ps(_mm256_loadu_ps(a+i),_mm256_loadu_ps(b+i),acc);float s=hsum_ps(acc);for(;i<n;++i)s+=a[i]*b[i];return s;
#else
...
void tensor_matvec(const GgufFile&file,const GgufTensor&t,const float*x,float*y,unsigned threads,WorkerPool*pool){
    ...
    if(t.type==GgmlType::Q4_0){if(cols%32)throw std::runtime_error("Q4_0 matrix row not block aligned: "+t.name);thread_local Q8Vector a;quantize_q8_into(x,cols,a);auto*A=reinterpret_cast<const GgufBlockQ4_0*>(p);auto nb=cols/32;parallel_rows(rows,threads,pool,[&](std::size_t r0,std::size_t r1){for(std::size_t r=r0;r<r1;++r)y[r]=dot_q4_q8(A+r*nb,a.q.data(),a.d.data(),nb);});}
