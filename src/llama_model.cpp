...
const std::vector<float>& LlamaModel::forward_batch_impl(const int*tokens,std::size_t B,bool compute_logits_last){
    ...
    std::vector<float>X(B*D),N(B*D),Q(B*D),KK(B*K),VV(B*K),ATT(B*D),PROJ(B*D),FF(B*F),OUT(B*D);
    ...
    for(std::size_t li=0;li<c_.n_layer;++li){auto&L=layers_[li];rms_batch(X,L.attn_norm_f,N);tensor_matmul_batch_v06(file_,*L.q,N.data(),Q.data(),B,threads_,&pool_);tensor_matmul_batch_v06(file_,*L.k,N.data(),KK.data(),B,threads_,&pool_);tensor_matmul_batch_v06(file_,*L.v,N.data(),VV.data(),B,threads_,&pool_);
        ...
        tensor_matmul_batch_v06(file_,*L.o,ATT.data(),PROJ.data(),B,threads_,&pool_);pool_.parallel_for(B*D,[&](std::size_t a,std::size_t z){for(std::size_t i=a;i<z;++i)X[i]+=PROJ[i];});rms_batch(X,L.ffn_norm_f,N);tensor_ffn_gate_up_batch_v06(file_,*L.gate,*L.up,N.data(),FF.data(),B,threads_,&pool_);tensor_matmul_batch_v06(file_,*L.down,FF.data(),OUT.data(),B,threads_,&pool_);pool_.parallel_for(B*D,[&](std::size_t a,std::size_t z){for(std::size_t i=a;i<z;++i)X[i]+=OUT[i];});
    }
